...
#####################################################################################################
### CONFIG VARS #####################################################################################
declare LLTEST_CMD="/app/srcds_run -game fof +map fof_fistful -insecure -norestart +sv_lan 1 -maxplayers 4";
declare LLTEST_NAME="gamesvr-fof-$(date '+%H%M%S')";
#####################################################################################################
#####################################################################################################
...
#####################################################################################################
### TESTS ###########################################################################################
# Stock FOF server tests
should_lack 'WARNING: Failed to load 32-bit libtinfo.so.5 or libncurses.so.5.' 'readline is enabled'
should_lack 'map load failed:' 'Server was able to load the map';
should_lack 'Server restart in 10 seconds' 'Server is not boot-looping';
should_have 'Setting breakpad minidump AppID = 295230' 'Sever started executing';
